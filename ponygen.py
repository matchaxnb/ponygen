...
def ponygen(
    dsn="mysql://root@localhost/information_schema",
    engine="mysql",
    schema_name="galette",
    outdir="/tmp",
    remove_prefix="",
  ...
  """
    ...
    for tbl_name, tbl_it in groupby(lines, key=lambda x: x[0]):
        ...
        if pkFields:
            ...
            pk = ""
        clst = class_template.format(
            className=className, tableName=tbl_name, fields="".join(fields), pk=pk
        )
        with open("/".join([outdir, ".".join([tbl_name, "py"])]), "w") as f:
            f.write(clst)
        mappings.append(
            "  from .{tblName} import {className}; {className}._table_ = ''.join([prefix, {className}._table_])".format(
                tblName=tbl_name, className=className
            )
        )
    conn.close()
